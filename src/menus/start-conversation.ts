...
import { readConfig, AGENTS_DIR } from "../config.js";
...
export async function startNewConversation(query?: string): Promise<void> {
  try {
    ...
    if (agents.length === 1) {
      ...
      const defaultAgent = config.defaultAgent ? (await agentRepository.read(config.defaultAgent)) : undefined;
      ...
      if (selectedAgentId !== "__none__") {
        const agent = await agentRepository.read(selectedAgentId);
        if (agent) {
          agentId = agent.id;
          if (agent.modelId) modelId = agent.modelId;
        }
    ...
    if (draft) {
      try {
        ...
        await conversationRepository.write(conv);
...
export async function continueConversation(): Promise<void> {
  ...
  try {
    const conversation = await conversationRepository.read(convId);
    if (!conversation) {
